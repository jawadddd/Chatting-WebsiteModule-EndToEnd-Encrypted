...
blowFish_S = [["d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
...
def BlowFish_text_to_hex(text):
    hex_value = ''
    for char in text:
        hex_value += hex(ord(char))[2:].zfill(2)
    return hex_value

def BlowFish_hex_to_text(hexadecimal):
    text = ''
    for i in range(0, len(hexadecimal), 2):
        hex_pair = hexadecimal[i:i + 2]
        decimal_value = int(hex_pair, 16)
        text += chr(decimal_value)
    return text

def BlowFish_binToHex(plainText):
    num = int(plainText, 2)
    hexa = hex(num)[2:]
    while len(hexa) < len(plainText) // 4:
        # maintain output length same length
        # as input by appending leading zeroes.
        hexa = "0" + hexa
    return hexa

def BlowFish_xor(a, b):
    a = BlowFish_hexToBin(a)
    b = BlowFish_hexToBin(b)
    ans = ""
    for i in range(len(a)):
        ans += chr(((ord(a[i]) - ord('0')) ^ (ord(b[i]) - ord('0'))) + ord('0'))
    ans = BlowFish_binToHex(ans)
    return ans

def BlowFish_addBin(a, b, modVal):
    ans = ""
    n1 = int(a, 16)
    n2 = int(b, 16)
    n1 = (n1 + n2) % modVal
...
def BlowFish_f(plainText, modVal):
    a = [""] * 4
    ans = ""
    for i in range(0, 8, 2):
        col = int(BlowFish_hexToBin(plainText[i: i + 2]), 2)
        a[i // 2] = blowFish_S[i // 2][col]
    ans = BlowFish_addBin(a[0], a[1], modVal)
    ans = BlowFish_xor(ans, a[2])
    ans = BlowFish_addBin(ans, a[3], modVal)
    return ans

def BlowFish_keyGenerate(key):
    j = 0
    for i in range(len(blowFish_P)):
        blowFish_P[i] = BlowFish_xor(blowFish_P[i], key[j: j + 8])
        print("subkey " + str(i + 1) + ": " + blowFish_P[i])
        j = (j + 8) % len(key)

def BlowFish_round(time, plainText, modVal):
    left = plainText[:8]
    right = plainText[8:16]
    left = BlowFish_xor(left, blowFish_P[time])
    fOut = BlowFish_f(left, modVal)
    right = BlowFish_xor(fOut, right)
    print("round " + str(time) + ": " + right + left)
    return right + left

def decryptBlowFish(modVal, plainText):
    print("In Decryption Text")
    print(plainText)
    print("Above was the plain text")
    for i in range(17, 1, -1):
        plainText = BlowFish_round(i, plainText, modVal)

    print("Post Procesing in Decryption")
    # postprocessing
    right = plainText[:8]
    left = plainText[8:16]
    right = BlowFish_xor(right, blowFish_P[1])
    left = BlowFish_xor(left, blowFish_P[0])
    return left + right

def encryptBlowFish(modVal, plainText):
    print("In Encrypt Function...")
    for i in range(16):
        plainText = BlowFish_round(i, plainText, modVal)
    right = plainText[:8]
    left = plainText[8:16]
    right = BlowFish_xor(right, blowFish_P[16])
    left = BlowFish_xor(left, blowFish_P[17])
    return left + right


@app.route('/receive_messageBlowFish', methods=['POST'])
def receive_messageBlowFish():
    modVal = 1
    print("hexadecimal: ")
    for i in range(32):
        modVal = modVal << 1
    ...
    print("This is the Text for Encryption")
    print(message_text)
    # text = "Mohsin Ali, I am a good boy!"
    hexadecimal = BlowFish_text_to_hex(message_text)
    print("hexadecimal: ")
    print(hexadecimal)
    ...
    # BlowFish_keyGenerate(key)
    print("-----Encryption-----")
    length = len(plainText)
    cipherText = ''
    for i in range(0, length, 16):
        substring = plainText[i:i + 16]
        cipherText += encryptBlowFish(modVal, substring)
    # cipherText = self.encrypt(plainText)
    print("Cipher Text: " + cipherText)

    return {'status': 'success', 'cipher_texts': cipherText}

@app.route('/decrypt_single_messageBlowFish', methods=['POST'])
def decrypt_single_messageBlowFish():
    modVal = 1
    print("hexadecimal: ")
    for i in range(32):
        modVal = modVal << 1
    ...
    cipherText = data.get('text', [])
    print("This is the Cipher Text After []")
    print(cipherText)
    cipherText = data.get('text', '')
    print("This is the Cipher Text After ''")
    print(cipherText)
    # cipherText = cipherText[2:-2]
    # cipherText = cipherText.strip("[]")
    # cipherText = eval(cipherText)
    # cipherText = cipherText[0]
    # S = "['d0b6480e39271dcbf1ab75e064622ce2']"
    # cipherText = cipherText.strip()
    ...
    decryptedPlainText = ''
    for i in range(0, length, 16):
        substring = cipherText[i:i + 16]
        decryptedPlainText += decryptBlowFish(modVal, substring)
    # cipherText = self.encrypt(plainText)
    print("Decrypted Plain Text: " + decryptedPlainText)
    pl_text = BlowFish_hex_to_text(decryptedPlainText)
    print("Plain Text converted is: ")
    print(pl_text)

    return {'status': 'success', 'decrypted_values': pl_text}

@app.route('/decrypt_single_message_SenderSideBlowFish', methods=['POST'])
def decrypt_single_message_SenderSideBlowFish():
    modVal = 1
    print("hexadecimal: ")
    for i in range(32):
        modVal = modVal << 1
...
def hex_to_text(hex_text):
    # Remove '0x' prefix if present and convert to ASCII
...
def receive_message():
    ...
    hex_message = string_to_combined_hex(message_text)
    len_hex_message=len(hex_message)
    ...
    string_builder=""
    cipher_texts = []
    for i in range(len_hex_message):
        cipher_text = bin2hex(encrypt(hex_message[i], rkb, rk))
        print("Cipher Text : ", cipher_text)
        cipher_texts.append(cipher_text)
 
    #     binary__=encrypt(cipher_text, rkb_rev, rk_rev)
    #     # print("binary::",binary__)
    #     text = bin2hex(encrypt(cipher_text, rkb_rev, rk_rev))
    #     # print("Text : ", text)

    #     converted_text = hex_to_text(text) 
    #     string_builder=string_builder + converted_text
    #     # print("Plain Text : ", converted_text)

    # print(string_builder)

    return {'status': 'success', 'cipher_texts': cipher_texts}

@app.route('/receive_message_decrypt', methods=['POST'])
def receive_message_decrypt():
    data = request.get_json() 
    # Accessing the 'text' field directly
    message_texts = data.get('text', [])

    text_values_nested = []
    for message_text_list in message_texts:
        text_values = message_text_list.get('text', [])
        text_values_nested.append(text_values)

    len_hex_message=len(text_values_nested)
    print("len_hex_message:", len_hex_message)
    ...
    decrypted_values_nested = []
    for text_values in text_values_nested:
        decrypted_values = []
        for cipher_text in text_values:
            # print("Decryption")
            binary_text = encrypt(cipher_text, rkb_rev, rk_rev)
            text = bin2hex(binary_text)
            converted_text = hex_to_text(text)
            # print(converted_text)
             
            cleaned_text = converted_text.replace('\x00', '')
            # print(cleaned_text)
            # Split words based on spaces
            # words = cleaned_text.split()

            # Concatenate words separated by commas
            concatenated_words = ''.join(cleaned_text)
            # print('concatenated_words:',concatenated_words)
            # Append the concatenated words to the result list
            decrypted_values.append(concatenated_words)

        decrypted_values_nested.append(decrypted_values)
        
    # Initialize a list to store modified messages
    modified_messages = []    
    # Loop through the original messages and replace the 'text' field with decrypted values
    for i, message_text_list in enumerate(message_texts):
        modified_message = message_text_list.copy()
        modified_message['text'] = decrypted_values_nested[i]
        modified_messages.append(modified_message)
        
    # print("modified_messages:",modified_messages)
    return {'status': 'success', 'message_text_arr': modified_messages}

@app.route('/decrypt_single_message', methods=['POST'])
def decrypt_single_message():
    data = request.get_json()
    print("Data:",data) 
    message_texts = data.get('text', [])
    print("message_texts:",message_texts)
   
    len_hex_message=len(message_texts)
    print("len_hex_message:", len_hex_message)
    ...
    decrypted_values = []
    for encrypted_values in message_texts:
        binary_text = encrypt(encrypted_values, rkb_rev, rk_rev)
        text = bin2hex(binary_text)
        converted_text = hex_to_text(text)
        cleaned_text = converted_text.replace('\x00', '')
        concatenated_words = ''.join(cleaned_text)
        # print("concatenated_words:",concatenated_words)
        decrypted_values.append(concatenated_words)
        # print("decrypted_values:",decrypted_values)
    print("Decrypted single Message Text Values Nested:", decrypted_values)
            
    return {'status': 'success', 'decrypted_values': decrypted_values}
